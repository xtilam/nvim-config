...
export async function watch() {
  ...
  async function startNVIM() {
    ...
    const restartPromise = utils.promise.wrapper();
    const waitClose = utils.promise.wrapper();
    let isClosed = false;
    ...
    if (!isClosed) {
      client?.command(":lua nvimConfigDebug.normalClose()");
      utils.delay(400).then(() => abortSignal.abort());
    }
...
async function findFreePort() {
  ...
  server.listen(0);
  const promise = utils.promise.wrapper();
